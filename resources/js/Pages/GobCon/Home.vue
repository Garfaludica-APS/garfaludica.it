...
<template>
	<Head>
		...
		<meta head-key="description" name="description" :content="$t('gobcon.description')">
		<meta head-key="keywords" name="keywords" :content="$t('gobcon.keywords')">
	...
	<main>
		<GobCon-Hero :appMark="appMark" :navItems="topNavigation" :eventYear="eventYear" eventDate="20-22 Giugno" eventLocation="Corfino, Villa Collemandina, Lucca" :bgImage="heroImage">
			<template #announcement>
				{{ $t('gobcon.announcement') }} <button @click="bookSection?.scrollIntoView({ behavior: 'smooth' })" class="font-semibold text-white inline hover:cursor-pointer"><span class="absolute inset-0" aria-hidden="true" />{{ $t('gobcon.notify_me') }} <span aria-hidden="true">&rarr;</span></button>
			</template>
			<template #actions>
				<button @click="bookSection?.scrollIntoView({ behavior: 'smooth' })" class="rounded-md bg-primary-btn px-3.5 py-2.5 text-sm font-semibold text-foreground shadow-xs hover:bg-primary-btn-hover focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary-btn-hover hover:cursor-pointer">{{ $t('gobcon.book_now') }}</button>
				<button @click="infoSection?.scrollIntoView({ behavior: 'smooth' })" class="text-sm/6 font-semibold text-foreground hover:cursor-pointer">{{ $t('gobcon.learn_more') }} <span aria-hidden="true">→</span></button>
			</template>
		</GobCon-Hero>

		<article ref="infoSection">
			<GobCon-Info :photo1="photo1" :photo2="photo2" :photo3="photo3" :photo4="photo4" :tdgLogo="tdgLogo" :eventYear="eventYear" :infoSections="infoSections" />
		</article>

		<section class="bg-gradient-to-b from-white from-50% to-indigo-400 to-50% pb-16 sm:pb-24 xl:pb-32">
			<GobCon-Hotels :hotels="hotels">
				<template #media>
					<video class="absolute inset-0 size-full rounded-2xl bg-gray-800 object-cover shadow-2xl" autoplay muted loop>
