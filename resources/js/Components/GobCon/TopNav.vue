	...
	<header class="absolute inset-x-0 top-0 z-50">
		...
		>
			...
			<div class="flex lg:hidden">
				<button
					type="button"
					class="-m-2.5 inline-flex items-center justify-center rounded-md p-2.5 text-dimmed-foreground"
					@click="mobileMenuOpen = true"
				>
					<span class="sr-only">{{
						$t("gobcon.open_main_menu")
					}}</span>
				...
				>
					...
					>
						<div class="flex items-center justify-between">
							...
							<button
								type="button"
								class="-m-2.5 rounded-md p-2.5 text-dimmed-foreground"
							...
							<div class="-my-6 divide-y divide-gray-500/25">
								<div class="space-y-2 py-6">
									<button
										...
										"
										class="-mx-3 block w-full text-left rounded-lg px-3 py-2 text-base/7 font-semibold text-foreground hover:bg-navbtn-hover hover:cursor-pointer"
									>
										{{ $t(item.name) }}
									</button>
								</div>
								<div class="py-6">
									<select
										class="-mx-3 block rounded-lg px-3 py-2.5 text-foreground hover:bg-navbtn-hover w-full text-base/7 font-semibold hover:cursor-pointer"
										v-model="currentLang"
										@change="
